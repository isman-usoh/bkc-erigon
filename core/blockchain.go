...
func ExecuteBlockEphemerallyForBKC(
	...
	pos, _ := engine.(consensus.PoS)
	//fmt.Printf("====txs processing start: %d====\n", block.NumberU64())
	for i, tx := range block.Transactions() {
		if chainConfig.IsChaophraya(block.Number().Uint64()) {
			if isSystemTx, err := pos.IsSystemTransaction(tx, block.Header(), chainReader); err != nil {
				return nil, err
			} else if isSystemTx {
				continue
			}
		...
		from, _ := tx.GetSender()
		ibs.Prepare(
			chainConfig.Rules(header.Number.Uint64(), 0),
			from,
			header.Coinbase,
			tx.GetTo(),
			[]common.Address{},
		...
		receipt, _, err := ApplyTransaction(chainConfig, blockHashFunc, engine, nil, gp, ibs, noop, header, tx, usedGas, nil, *vmConfig)
		if writeTrace {
	...
	if !vmConfig.ReadOnly {
		// We're doing this hack for BKC to avoid changing consensus interfaces a lot. BKC modifies txs and receipts by appending
...
func SysCallContract(contract libcommon.Address, data []byte, chainConfig *chain.Config, ibs *state.IntraBlockState, header *types.Header, engine consensus.EngineReader, constCall bool) (result []byte, err error) {
	...
	blockContext := NewEVMBlockContext(header, chainConfig, GetHashFn(header, nil), engine, author)
...
func SysCreate(contract libcommon.Address, data []byte, chainConfig chain.Config, ibs *state.IntraBlockState, header *types.Header) (result []byte, err error) {
	...
	author := &contract
	txContext := NewEVMTxContext(msg)
	blockContext := NewEVMBlockContext(header, &chainConfig, GetHashFn(header, nil), nil, author)
	evm := vm.NewEVM(blockContext, txContext, ibs, &chainConfig, vmConfig)
